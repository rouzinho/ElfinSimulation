...
using namespace arm_controllers;
{

		ReactiveController::~ReactiveController()
		{
			command_sub_.shutdown();
			command_behavior.shutdown();
		}

		bool ReactiveController::init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
  		{
			loop_count_ = 0;
			cmd = "start";
			...
			command_sub_ = n.subscribe<std_msgs::Float64MultiArray>("command", 1, &ReactiveController::commandCB, this);
			command_behavior = n.subscribe<std_msgs::String>("behavior", 1, &ReactiveController::commandBH, this);
		...
		void ReactiveController::starting(const ros::Time& time)
		{
			// get joint positions
			for(size_t i=0; i<n_joints_; i++)
			{
		...
		void ReactiveController::commandCB(const std_msgs::Float64MultiArrayConstPtr& msg)
		{
		...
		void ReactiveController::commandBH(const std_msgs::String::ConstPtr& msg)
		{
			cmd = msg->data.c_str();
		}

  		void ReactiveController::update(const ros::Time& time, const ros::Duration& period)
  		{
			...
			{
				balancing(time,period);
			}

  		}

		void ReactiveController::balancing(const ros::Time& time, const ros::Duration& period)
		{
		...
		void ReactiveController::slowing(const ros::Time& time, const ros::Duration& period)
		{
			std::vector<double> & commands = *commands_buffer_.readFromRT();
			double dt = period.toSec();
			double q_cmd_old;
			...
			for (size_t i=0; i<n_joints_; i++)
			{
				...
				//q_cmd_(i) = commands[i];
				q_cmd_(i) = D2R*sin(PI/2*t);

				enforceJointLimits(q_cmd_(i), i);
				// qdot_cmd_(i) = ( q_cmd_(i) - q_cmd_old )/dt;
				qdot_cmd_(i) = D2R*PI/2*cos(PI/2*t);
  		...
  		void ReactiveController::stopping(const ros::Time& time) { }

		void ReactiveController::enforceJointLimits(double &command, unsigned int index)
		{
